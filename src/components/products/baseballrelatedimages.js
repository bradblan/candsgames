...
const BaseballRelatedImages = () => (
  <StaticQuery
    query={graphql`
      query {
        allGalleryJson(
          filter: { key: { eq: "baseball" }, featured: { eq: false } }
        ) {
          edges {
            node {
              alt
        ...
        allFile(filter: { relativeDirectory: { eq: "products/baseball" } }) {
          edges {
            node {
              childImageSharp {
                fluid(maxWidth: 300) {
...
export default BaseballRelatedImages
